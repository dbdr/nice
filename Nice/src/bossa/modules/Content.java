...
class Content
{
  ...
  */
  Stream[] getClasses()
  {
    ...
    if (!sourceRead)
      {
	res = compiled.getClasses();
	File dispatchFile = new File(output, "dispatch.class");
	try{
	  res[res.length - 1] = new Stream(new FileInputStream(dispatchFile), 
					   "dispatch.class");
	}
      ...
      res = DirectoryCompiledContent.getClasses(output, true);
