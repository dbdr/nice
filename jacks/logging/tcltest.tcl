...
namespace eval tcltest {
    ...
    set procList [list test cleanupTests saveState restoreState \
	    ...
	    loadTestedCommands normalizePath resultmatch logfile autorecovery]
    ...
    # Do not use string matching by default
    if {![info exists resultmatch]} {
        variable resultmatch 0
    }

    # Do not log test results by default
    ...
    # The name of crashed test during last run
    if {![info exists autorecovery]} {
...
proc ::tcltest::cleanupTests {{calledFromAllFile 0} {testFileName DEFAULT}} {

    if {$testFileName == "DEFAULT"} {
...
# We also log the final pass/fail numbers
            ::tcltest::logEntry "$index $::tcltest::numTests($index)"
	}
...
# We don't want any "file left behind messages to be displayed
...
proc ::tcltest::test {name description script expectedAnswer args} {
    ...
    # See if we are in the middle of an autorecovery. If we are
    # then keep skipping test until we find the one that crashed.

    if {$::tcltest::autorecovery != ""} {
        if {$::tcltest::autorecovery == $name} {
            # This is the test that crashed last time, we
            # skip it and run the next test normally
            set ::tcltest::autorecovery ""
        }
        incr ::tcltest::numTests(Skipped)
        DebugDo 1 {::tcltest::AddToSkippedBecause autorecovery}
        return
    }
    ...
    ::tcltest::logEntry "started $name"
    set code [catch {uplevel $script} actualAnswer]
    ::tcltest::logEntry "finished $name"

    if {$tcltest::resultmatch} {
        set gotExpected [string match $expectedAnswer $actualAnswer]
    } else {
        set gotExpected [string equal $expectedAnswer $actualAnswer]
    }

    # 0 for normal termination, 2 for a call to return inside tcltest::test
    if {($code == 0 || $code == 2) && $gotExpected} {
	incr ::tcltest::numTests(Passed)
        ::tcltest::logEntry "RESULT $name PASSED"
	...
	set ::tcltest::currentFailure true
        ::tcltest::logEntry "RESULT $name FAILED"
...
# resultmatch --
#
#	Set the current resultmatch status.
#	If resultmatch is activated, then test
#	results will be compared to the expected
#	result with string match instead of
#	a string compare.
#
# Arguments:
#	boolean to indicate if string match
#	should be used for future tests.
#
# Results:
#	Changes the way the result is checked.

proc ::tcltest::resultmatch { enabled } {
    set ::tcltest::resultmatch $enabled
}

# logfile --
#
#	Set or query the current log file. When
#	a logfile is used, the results of each
#	test will be written to the log file.
#	You can take a log file and compare it
#	to another log file to find differences.
#
# Arguments:
...
proc ::tcltest::logfile { {filename __QUERY} } {
    if {$filename == "__QUERY"} {
        return $::tcltest::logfilename
    } else {
        # If the file name is not already fully qualified
        # we need to do that now in case a test calls cd
        # before running the first test.

        if {[file pathtype $filename] != "absolute"} {
	    set ::tcltest::logfilename [file join [pwd] $filename]
        } else {
            set ::tcltest::logfilename $filename
        }

        #puts "set log file to $::tcltest::logfilename"

        if {$::tcltest::logfile != ""} {
            close $::tcltest::logfile
            set ::tcltest::logfile ""
        }
    }
}
...
# logEntry --
#
#	Add an entry to the logfile, this is a
#	private proc that should only be used
#	inside the tcltest module.
#
# Arguments:
#	none.
#
...
proc ::tcltest::logEntry { str } {
    if {$::tcltest::logfilename != ""} {
        # If the log file has not been opened yet, open it now.
        if {$::tcltest::logfile == ""} {
             #puts "opening new log file $::tcltest::logfilename"
             set ::tcltest::logfile [open $::tcltest::logfilename w]
             fconfigure $::tcltest::logfile -buffering line
# Dont write times out to the log file, it just makes it harder to diff
#             puts $::tcltest::logfile \
                 "Started regression tests on [clock format [clock seconds]]"
        }

        puts $::tcltest::logfile $str
    }
}

# logClose --
#
#	Close down the log file, this is a
#	private proc that should only be used
#	inside the tcltest module.
#
# Arguments:
...
proc ::tcltest::logClose { } {
    if {$::tcltest::logfilename != ""} {
# Dont write times out to the log file, it just makes it harder to diff
#        ::tcltest::logEntry "Finished tests on [clock format [clock seconds]]"
        close $::tcltest::logfile
    }
}
...
#	crash recovery. This feature will
#	skip all test up to and including
#	the one that crashed during the
#	previous run.
#
# Arguments:
#	enabled : boolean to indicate if
#	the autorecover will be used.
#
# Results:
#	Will open the previous log file
#	to see if it crashed. Will error
#	out if a log file is not set.

#	we currently do not try to recover
#	pass/fail/skip stats from the log file
#	after a crash. That might be a
#	good thing to add in the future.

proc ::tcltest::autorecovery { enabled } {
    if {$enabled} {
        if {$::tcltest::logfilename == ""} {
            error "no log file was specified"
        }

        # See if the system crashed last time we ran the
        # tests. In this case we want to find out what
        # test it crashed on so that we can start over
        # from there.

        #puts "checking for last run logfile $::tcltest::logfilename"

        if {[file exists $::tcltest::logfilename]} {
            set ::tcltest::logfile [open $::tcltest::logfilename r+]
            fconfigure $::tcltest::logfile -buffering line

            # Tcl's seek command seem to be broken, this should
            # go to the end of the file minus 400 bytes, but no dice.
            #seek $::tcltest::logfile -400 end

            # Get the last non empty line
            set lines [split [read $::tcltest::logfile] \n]
            ...
            # If the last line is something like "started foo-1.3" then
            # we crashed during test foo-1.3 during the last run.

            #puts "Last line of $::tcltest::logfilename is"
            #puts "\"$line\""

            if {[lindex $line 0] == "started"} {
                set ::tcltest::autorecovery [lindex $line 1]
                ::tcltest::logEntry "AUTORECOVERY $::tcltest::autorecovery on [clock format [clock seconds]]"
                puts $::tcltest::outputChannel "!!!! Last session crashed during \"$::tcltest::autorecovery\" performing autorecovery"

            } else {
                # We did not find a crashed test, just run normally
                close $::tcltest::logfile
                set ::tcltest::logfile ""
            }
        }
